<%@ Page Title="Blackwater Medical Centre | Downloads" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Download.aspx.cs" Inherits="BlackwaterMedicalCentre.Download" %>

<asp:Content ID="Content1" ContentPlaceHolderID="HeadContent" runat="server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <table>
        <tr>
            <th>
                Documents
            </th>
        </tr>
        <tr>
            <td>
                <a style="width: 10px;" href="/Downloads/BW_Patient_Participation_Survey.doc">Download Patient Participation Survey</a>
            </td>
        </tr>
        <tr>
            <td>
                <a style="width: 10px;" href="/Downloads/Blackwater_Results_Mar_2012.pdf">Download the Blackwater Results for March 2012</a>
            </td>
        </tr>
        <tr>
            <td>
                <a style="width: 10px;" href="/Downloads/Local_Patient_Participation_Report_Mar_2012.doc">Download the Patient Participation Report March 2012</a>
            </td>
        </tr>
        <tr>
            <td>
                <a style="width: 10px;" href="/Downloads/Blackwater_Medical_Centre_Appointment_System.doc">View how the appointment system works</a>
            </td>
        </tr>
        <tr>
            <td>
                <a style="width: 10px;" href="/Downloads/PrivateFeesSept11.doc">View our private fees</a>
            </td>
        </tr>
        <tr>
            <td>
                <a style="width: 10px;" href="/Downloads/NICOR_patient_reports_PR_final.doc">Heart patients receive major awareness boost with first patient-focused heart attack and heart failure audit reports</a>
            </td>
        </tr>
        <tr>
            <td>
                <a style="width: 10px;" href="/Downloads/Primary_Care_Foundation_Survey_Results.doc">View the Primary Care Foundation survey results</a>
            </td>
        </tr>
         <tr>
            <td>
                <a style="width: 10px;" href="/Downloads/Blackwater_Results_Mar_2013.pdf">Download the Blackwater Results for March 2013</a>
            </td>
        </tr>
    </table>
</asp:Content>
