<%@ Page Title="Blackwater Medical Centre | Opening times" Language="C#" MasterPageFile="~/Site.Master"
    AutoEventWireup="true" CodeBehind="Times.aspx.cs" Inherits="BlackwaterMedicalCentre.Times" %>

<asp:Content ID="Content1" ContentPlaceHolderID="HeadContent" runat="server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <h2>
        Surgery Opening Hours - <a href="default.aspx#1">Click Here </a>
    </h2>
    <h3>
        Maldon Sugery</h3>
    <p>
        As of the 25th November 2013, the practice will not be providing Saturday morning
        surgeries due to directives from NHS England.</p>
    Princes Road, Maldon, Essex CM9 5GP<br />
    <strong>Dr Robin Roper</strong>
    <table style="border: 1px solid black">
        <tr>
            <td style="border: 1px solid black">
                Monday
            </td>
            <td style="border: 1px solid black">
                07.00 to 10.30
            </td>
            <td style="border: 1px solid black">
                14.00 to 17.00
            </td>
        </tr>
        <tr>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                Tuesday
            </td>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                08.30 to 10.00
            </td>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                14.00 to 17.00
            </td>
        </tr>
        <tr>
            <td style="border: 1px solid black">
                Wednesday
            </td>
            <td style="border: 1px solid black">
                08.30 to 10.10
            </td>
            <td style="border: 1px solid black">
                15.40 to 18.00
            </td>
        </tr>
    </table>
    <strong>Dr Martin Haeger</strong>
    <table style="border: 1px solid black">
        <tr>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                Tuesday
            </td>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                08.30 to 11.00
            </td>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                14.30 to 17.30
            </td>
        </tr>
        <tr>
            <td style="border: 1px solid black">
                Wednesday
            </td>
            <td style="border: 1px solid black">
                07.00 to 10.20
            </td>
            <td style="border: 1px solid black">
                15.40 to 17.00
            </td>
        </tr>
        <tr>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                Thursday
            </td>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                09.00 to 11.40
            </td>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                14.00 to 17.30
            </td>
        </tr>
    </table>
    <strong>Dr Vittorio Teatino</strong>
    <table style="border: 1px solid black">
        <tr>
            <td style="border: 1px solid black">
                Wednesday
            </td>
            <td style="border: 1px solid black">
                07.30 to 08.30 and 09.00 to 09.50
            </td>
            <td style="border: 1px solid black">
                15.40 to 18.00
            </td>
        </tr>
        <tr>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                Thursday
            </td>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                07.30 to 08.30 and 09.00 to 09.50
            </td>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                15.40 to 18.00
            </td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">
                Friday
            </td>
            <td style="border: 1px solid black;">
                07.00 to 08.00 and 09.00 to 10.30
            </td>
            <td style="border: 1px solid black;">
                14.00 to 17.00
            </td>
        </tr>
    </table>
    <strong>Dr Linda Lim</strong>
    <table style="border: 1px solid black">
        <tr>
            <td style="border: 1px solid black">
                Monday
            </td>
            <td style="border: 1px solid black">
                08.30 to 11.00
            </td>
            <td style="border: 1px solid black">
                14.30 to 17.00
            </td>
        </tr>
        <tr>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                Tuesday
            </td>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                08.30 to 11.00
            </td>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                14.00 to 16.00
            </td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">
                Wednesday
            </td>
            <td style="border: 1px solid black;">
                07.00 to 10.20
            </td>
            <td style="border: 1px solid black;">
                15.40 to 19.00
            </td>
        </tr>
        <tr>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                Thursday
            </td>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                08.30 to 11.00
            </td>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                14.30 to 17.30
            </td>
        </tr>
    </table>
    <strong>Dr Arde Geranmayeh</strong>
    <table style="border: 1px solid black">
        <tr>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                Tuesday
            </td>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                10.30 to 13.00
            </td>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                14.30 to 17.30
            </td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">
                Wednesday
            </td>
            <td style="border: 1px solid black; background-color: Light Gray">
               07.00 to 10.20
            </td>
            <td style="border: 1px solid black;">
                15.40 to 19.00
            </td>
        </tr>
        <tr>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                Thursday
            </td>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                08.30 to 11.00
            </td>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                14.30 to 16.30
            </td>
        </tr>
        <tr>
            <td style="border: 1px solid black">
                Friday
            </td>
            <td style="border: 1px solid black">
                08.30 to 11.30
            </td>
            <td style="border: 1px solid black">
                14.30 to 18.00
            </td>
        </tr>
    </table>
    <strong>Dr Tim Wright</strong>
    <table style="border: 1px solid black">
        <tr>
            <td style="border: 1px solid black">
                Monday
            </td>
            <td style="border: 1px solid black">
                08.30 to 10.30
            </td>
            <td style="border: 1px solid black">
                14.00 to 17.00
            </td>
        </tr>
        <tr>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                Tuesday
            </td>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
                07.00 to 09.40
            </td>
            <td style="border: 1px solid black; background-color: #DFDDFF;">
            </td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">
                Wednesday
            </td>
            <td style="border: 1px solid black;">
                08.30 to 10.30
            </td>
            <td style="border: 1px solid black;">
                15.40 to 17.00
            </td>
        </tr>
        <tr>
            <td style="border: 1px solid black;background-color: #DFDDFF;">
                Friday
            </td>
            <td style="border: 1px solid black;background-color: #DFDDFF;">
               08.30 to 11.10
            </td>
            <td style="border: 1px solid black;background-color: #DFDDFF;">
                14.00 to 17.00
            </td>
        </tr>
    </table>
    <strong>Dr Goswami</strong>
    <table style="border: 1px solid black">
        <tr>
            <td style="border: 1px solid black;">
                Tuesday
            </td>
            <td style="border: 1px solid black;">
                08.30 to 11.30
            </td>
             <td style="border: 1px solid black;background-color: #DFDDFF;">
                14.30 to 17.00
            </td>
        </tr>
        <tr>
            <td style="border: 1px solid black;background-color: #DFDDFF;">
                Wednesday
            </td>
            <td style="border: 1px solid black;background-color: #DFDDFF;">
                08.30 to 11.30
            </td>
             <td style="border: 1px solid black;background-color: #DFDDFF;">
                14.30 to 17.00
            </td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">
                Thursday
            </td>
            <td style="border: 1px solid black;">
                08.30 - 11.30
            </td>
            <td style="border: 1px solid black;background-color: #DFDDFF;">
                14.30 to 17.00
            </td>
        </tr>
        <tr>
            <td style="border: 1px solid black;background-color: #DFDDFF;">
                Friday
            </td>
            <td style="border: 1px solid black;background-color: #DFDDFF;">
               08.30 to 11.30
            </td>
            <td style="border: 1px solid black;background-color: #DFDDFF;">
                14.30 to 17.00
            </td>
        </tr>
    </table>
    <h3>
        Heybridge Sugery</h3>
    Goldring House, Rowan Drive, Heybridge, Essex, CM9 4BW
      <table style="border: 1px solid black">
        <tr>
            <td style="border: 1px solid black">Monday
            </td>
            <td style="border: 1px solid black">08.30 to 10.30
            </td>
            <td style="border: 1px solid black">Dr Wright
            </td>
        </tr>
        <tr>
            <td style="border: 1px solid black">Tuesday
            </td>
            <td style="border: 1px solid black">08.30 to 11.30
            </td>
            <td style="border: 1px solid black;">Dr Haeger
            </td>
        </tr>
        <tr>
            <td style="border: 1px solid black; background-color: #DFDDFF;">Wednesday
            </td>
            <td style="border: 1px solid black; background-color: #DFDDFF;">08.30 to 11.00
            </td>
            <td style="border: 1px solid black; background-color: #DFDDFF;">Dr Roper
            </td>
        </tr>
        <tr>
            <td style="border: 1px solid black"></td>
            <td style="border: 1px solid black">08.40 to 11.10
            </td>
            <td style="border: 1px solid black">Dr Teatino
            </td>
        </tr>
        <tr>
            <td style="border: 1px solid black; background-color: #DFDDFF;">Thursday
            </td>
            <td style="border: 1px solid black; background-color: #DFDDFF;">08.30 to 11.30
            </td>
            <td style="border: 1px solid black; background-color: #DFDDFF;">Dr Geranmayeh
            </td>
        </tr>
        <tr>
            <td style="border: 1px solid black; background-color: #DFDDFF;"></td>
            <td style="border: 1px solid black; background-color: #DFDDFF;">14.00 to 17.00
            </td>
            <td style="border: 1px solid black; background-color: #DFDDFF;">Dr Teatino
            </td>
        </tr>
        <tr>
            <td style="border: 1px solid black">Friday
            </td>
            <td style="border: 1px solid black">07.00 to 08.00
            </td>
            <td style="border: 1px solid black">Dr Teatino
            </td>
        </tr>
          <tr>
            <td style="border: 1px solid black">&nbsp;
            </td>
            <td style="border: 1px solid black">09.00 to 10.30
            </td>
            <td style="border: 1px solid black">Dr Teatino
            </td>
        </tr>
    </table>
    <h3>
        <a name="1"></a>Emergencies - Out Of Hours
    </h3>
    <p>
        We wish to stress that out-of-hours services are strictly for emergencies only.
        From 6.30pm until 8.00am Monday to Friday and from 6.30pm Friday until 8.00am Monday,
        patients can contact the out-of-hours service by telephoning the NHS on 111.</p>
    <p>
        You can also contact NHS Direct, a 24 hour nurse-led information and advice service,
        by telephoning 111 or online at <a href="http://www.nhsdirect.nhs.uk/" target="_blank">
            www.nhsdirect.nhs.uk</a></p>
    <h3>
        <a name="2" id="2"></a>Disabled Access/Parking</h3>
    <p>
        Wheelchair access can be made through the front door and the toilets are suitable
        for disabled patients. Designated parking is available.</p>
</asp:Content>
